...
public class ViewServiceManager {
    ...
    public synchronized void setViewContribution(IViewContribution viewContribution) {
        waitUntilInitialized();
        System.out.println("ViewServiceManager.addingService(): " + viewContribution);
        ...
        for (int pos = 0; pos < viewContainer.getTabCount(); pos++) {
            if (position <= positionMap.get(viewContainer.getComponentAt(pos))) {
                viewContainer.insertTab(viewContribution.getName() + " ", viewContribution
                        .getIcon(), view, viewContribution.getName(), pos);
                isInserted = true;
                break;
            }
        }
        if (!isInserted) {
            viewContainer.insertTab(viewContribution.getName() + " ", viewContribution.getIcon(),
                    view, viewContribution.getName(), viewContainer.getTabCount());
        }

        serviceCount += 1;
    }

    public synchronized void unsetViewContribution(IViewContribution viewContribution) {
        JComponent view = (viewContribution).getView();
