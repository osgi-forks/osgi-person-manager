...
public class ExtenderBundleTracker extends AbstractBundleTracker {
    ...
    protected void tracked(Bundle bundle) {
        String className = (String) bundle.getHeaders().get("Action-Contribution");
        if (className != null) {
            Class clazz;
            try {
                ...
                try {
                    ServiceRegistration serviceRegistration = context.registerService(
                            "com.siemens.ct.pm.application.service.IActionContribution", clazz
                                    .newInstance(), null);
                    serviceMap.put(bundle.getSymbolicName(), serviceRegistration);
                    System.out.println("Extender: Action Contribution Service registered for: "
                            + clazz.getName());
    ...
    protected void untracked(Bundle bundle) {
        ServiceRegistration serviceRegistration = serviceMap.get(bundle.getSymbolicName());
